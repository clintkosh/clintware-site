...
function livePrompt(){
  ...
  return head('Natural-language operations','Live Prompt','Type what changed. Exa-backed context and the current CRM record are used to interpret the update, propose precise record/card changes, and wait for approval before writing anything.')+
    ...
    '<div class="grid g2" style="margin-top:16px"><div class="card"><div class="eyebrow">Selected customer</div><h2>'+e(S.customer?.name||'No customer selected')+'</h2><p class="muted">Examples: “Okta SSO is failing for one production role; mark the integration at risk and create a validation action.” “The customer confirmed median advanced-investigation time is 72 minutes.” “Research current public guidance for SAML troubleshooting and tell me which parts of our diagnostic plan need review.”</p>'+
    ...
    '<div class="field"><label>Exa context</label><select id="prompt-research" class="select"><option value="auto">Auto: use Exa only when the wording calls for public/external verification</option><option value="on">On: always add public technical-support / integration / security context</option><option value="off">Off: CRM context only</option></select><small>Before Exa is called, the control plane generates and scrubs a de-identified public search query. Raw customer CRM text is not sent directly to Exa.</small></div>'+
...
function liveAssistant(){
  ...
  return head('Consent-gated call support','Live Customer Assistant','White-label live call assistant for Technical Customer Engineering work. It can transcribe opted-in live machine audio or past recordings, gate suggestions to the selected customer, and answer direct questions on demand.')+
