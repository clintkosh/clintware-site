...
async function load(id){let q=id?"?customer="+encodeURIComponent(id):"",state=await api("/state"+q),kb=await maybe("/kb");S=state;K=kb||{articles:[],latest:[],trending:[],config:{},usageSignalAvailable:false};if(state.access?.authenticated){let out=await Promise.all([maybe("/integrations/jira/status"),maybe("/integrations/confluence/status"),maybe("/control-plane/status")]);I={jira:out[0],confluence:out[1],controlPlane:out[2]}}else I={jira:null,confluence:null,controlPlane:null};render();return S}
