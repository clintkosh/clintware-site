...
let S={customer:null,customers:[],records:[],workspace:null,access:{authenticated:false,mode:"guest"}},K={articles:[],latest:[],trending:[],config:{},usageSignalAvailable:false},I={jira:null,confluence:null,controlPlane:null},tab="command",scen=new Set(),theme=localStorage.getItem("n7theme")||"dark";
...
async function load(id){let q=id?"?customer="+encodeURIComponent(id):"";let out=await Promise.all([api("/state"+q),api("/kb"),maybe("/integrations/jira/status"),maybe("/integrations/confluence/status"),maybe("/control-plane/status")]);S=out[0];K=out[1];I={jira:out[2],confluence:out[3],controlPlane:out[4]};render()}
