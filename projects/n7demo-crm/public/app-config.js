...
async function load(id){let q=id?"?customer="+encodeURIComponent(id):"";let state=await api("/state"+q),out=await Promise.all([maybe("/kb"),maybe("/integrations/jira/status"),maybe("/integrations/confluence/status"),maybe("/control-plane/status")]);S=state;K=out[0]||{articles:[],latest:[],trending:[],config:{},usageSignalAvailable:false};I={jira:out[1],confluence:out[2],controlPlane:out[3]};render();return S}
