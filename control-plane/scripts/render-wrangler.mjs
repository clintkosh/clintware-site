...
if (!Array.isArray(base.routes) || !base.routes.some((route) => route.pattern === "mcp.clintware.com" && route.custom_domain === true)) {
  throw new Error("Production config must retain mcp.clintware.com as a custom domain.");
}

fs.writeFileSync(path.join(root, "wrangler.generated.jsonc"), JSON.stringify(base, null, 2) + "\n");
console.log("Rendered wrangler.generated.jsonc with OAUTH_KV binding and mcp.clintware.com custom domain.");
