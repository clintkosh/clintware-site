...
from agentbridge_node.executor import execute_pack_path,rollback
...
class ExecutorTests(unittest.TestCase):
    def test_write_dod_and_rollback(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            pack=Path(td)/"x.abpack"; save_abpack(manifest,pack); cfg=Config.load(); cfg.data["policy"]["file.write"]="always"; cfg.save()
            result=execute_pack_path(pack,cfg); self.assertEqual(result["status"],"passed"); self.assertEqual(target.read_text(),"after")
    ...
    def test_never_cannot_be_approved(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            cfg=Config.load(); cfg.data["policy"]["git.push"]="never"; cfg.save()
            result=execute_pack_path(pack,cfg,approved=True); self.assertEqual(result["status"],"denied")
