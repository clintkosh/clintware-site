...
export const BASE_HELP={
  ...
  getting_started:[
    ...
    {id:"local-ai",title:"Local AI manager",body:"Use `quillgeist local-ai status|fit|route|fallback|plan|benchmark|curve|workload` for installed-model operations. `autofit` creates a measured proposal; `apply` activates only the Quillgeist profile; `verify` checks it and can restore the previous profile after a measured regression. `gateway` serves the OpenAI-compatible local chat surface on loopback only. These commands do not implicitly download models or tune hardware."},
    {id:"delta-state",title:"Delta-state context",body:"For repeated project work, Quillgeist can keep project-scoped context locally on the Node, preserve exact anchors, ingest only new deltas, bound active working state, and retrieve relevant older context when needed. The local `quillgeist-state` command supports ingest, show, status, and explicit reset operations; `quillgeist-plan --no-state` disables state compaction for a planning call."},
  ...
  glossary:[
    ...
    {term:"Delta-state context",definition:"Project-scoped local context that protects exact anchors, keeps a bounded active working set, stores older context cold, and rehydrates only history relevant to the next step."},
