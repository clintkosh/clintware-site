...
function restrictedDataCategory(text){
  ...
  if(/-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----/i.test(value)||/\b(?:password|passwd|api[_ -]?key|access[_ -]?token|client[_ -]?secret)\s*[:=]\s*\S+/i.test(value)||/\bsk-[A-Za-z0-9_-]{12,}\b/.test(value)||/\bAKIA[A-Z0-9]{16}\b/.test(value)||/\bBearer\s+[A-Za-z0-9._~+\/-]+=*/i.test(value))return"access_credentials";
  if(/\b\d{3}-\d{2}-\d{4}\b/.test(value))return"government_identifier";
  const cardCandidates=value.match(/(?:\d[ -]?){13,19}/g)||[];if(cardCandidates.some(luhn))return"payment_card_data";
  if(/\b(?:protected health information|medical record|health record|patient (?:name|id|dob)|diagnos(?:is|ed)|HIPAA|PHI)\b/i.test(value))return"protected_health_information";
  return null;
}
...
function openApiDocument(){return{openapi:"3.1.0",info:{title:"Quillgeist Public API",version:API_VERSION,description:"Privacy-conscious prompt/context compaction and aggregate Quillgeist impact metrics. Do not submit access credentials, payment-card data, government identifiers, or protected health information."},servers:[{url:"https://quillgeist.clintware.com"}],paths:{"/api/v1/compact":{post:{operationId:"compactContext",summary:"Compact prompt or execution context while preserving high-signal evidence",requestBody:{required:true,content:{"application/json":{schema:{type:"object",required:["text"],properties:{text:{type:"string",maxLength:MAX_TEXT_CHARS,description:"Task-specific text only. Restricted data is not accepted."},threshold_chars:{type:"integer",minimum:500,maximum:50_000,default:DEFAULT_THRESHOLD_CHARS},max_chars:{type:"integer",minimum:1_000,maximum:100_000,default:DEFAULT_MAX_CHARS},record_aggregate_metrics:{type:"boolean",default:true,description:"Set false to disable the aggregate compaction event for this request."}}}}},responses:{"200":{description:"Compacted context and estimated token metrics"},"422":{description:"Restricted data detected; request not processed"}}}},"/api/v1/impact":{get:{operationId:"getProductImpact",summary:"Get aggregate Quillgeist impact metrics and trends",parameters:[{name:"days",in:"query",schema:{type:"integer",minimum:7,maximum:90,default:30}}],responses:{"200":{description:"Aggregate, privacy-safe product metrics"}}}}}};}
...
export async function handlePublicApi(request,env,ctx){
  ...
  if(request.method==="POST"&&url.pathname==="/api/v1/compact"){
    try{
      const body=await readJsonLimited(request),text=String(body.text??"");if(!text.trim())return json({error:"text_required"},400,cors);if(text.length>MAX_TEXT_CHARS)return json({error:"text_too_large",max_chars:MAX_TEXT_CHARS},413,cors);
      const restricted=restrictedDataCategory(text);if(restricted)return json({error:"restricted_data_not_accepted",category:restricted,message:"Remove restricted data before using Quillgeist public compaction."},422,cors);
      const result=compactText(text,body),recordMetrics=body.record_aggregate_metrics!==false;if(recordMetrics)ctx.waitUntil(recordApiCompaction(env,result.metrics,"public_api"));
      return json({version:API_VERSION,output:result.output,metrics:result.metrics,privacy:{content_stored_by_quillgeist:false,content_logged_to_quillgeist_telemetry:false,aggregate_metrics_recorded:recordMetrics,aggregate_metrics_opt_out:"Set record_aggregate_metrics to false."}},200,cors);
    }catch(error){return json({error:String(error?.message||"bad_request")},Number(error?.status||400),cors);}
  }
  return null;
}
...
function createQuillgeistMcpServer(env){
  ...
  server.registerTool("quillgeist_compact_context",{title:"Compact context with Quillgeist",description:"Reduce repetitive or low-signal prompt, log, transcript, or execution context before sending it to another model. Preserve error-adjacent evidence plus the beginning and end of long inputs. Do not submit access credentials, payment-card data, government identifiers, or protected health information. By default this tool records an aggregate compaction event containing method/source and estimated token counts, but not the submitted text; set record_aggregate_metrics=false to opt out for the call.",inputSchema:{text:z.string().min(1).max(MAX_TEXT_CHARS).describe("Task-specific prompt or context to compact. Restricted data is not accepted."),threshold_chars:z.number().int().min(500).max(50_000).optional().describe("Do not compact below this character count. Default 6000."),max_chars:z.number().int().min(1_000).max(100_000).optional().describe("Target maximum compacted character count. Default 24000."),record_aggregate_metrics:z.boolean().optional().describe("Set false to disable the aggregate compaction telemetry event for this call. Default true.")},annotations:{readOnlyHint:false,destructiveHint:false,idempotentHint:false,openWorldHint:false}},async({text,threshold_chars,max_chars,record_aggregate_metrics})=>{
    const restricted=restrictedDataCategory(text);if(restricted)return{isError:true,content:[{type:"text",text:JSON.stringify({error:"restricted_data_not_accepted",category:restricted,message:"Remove restricted data before using Quillgeist public compaction."})}]};
    const result=compactText(text,{threshold_chars,max_chars}),recordMetrics=record_aggregate_metrics!==false;if(recordMetrics)await recordApiCompaction(env,result.metrics,"chatgpt_mcp");
    return{content:[{type:"text",text:JSON.stringify({output:result.output,metrics:result.metrics,privacy:{content_logged_to_quillgeist_telemetry:false,aggregate_metrics_recorded:recordMetrics,aggregate_metrics_opt_out:"Set record_aggregate_metrics to false."}})}]};
  });
