...
function createQuillgeistMcpServer(env){
  ...
  server.registerTool("quillgeist_compact_context",{title:"Compact context with Quillgeist",description:"Reduce repetitive or low-signal prompt, log, transcript, or execution context before sending it to another model. Preserve error-adjacent evidence plus the beginning and end of long inputs. Return compacted text and estimated token reduction. Quillgeist does not store submitted content in its application telemetry.",inputSchema:{text:z.string().min(1).max(MAX_TEXT_CHARS).describe("Prompt or context to compact."),threshold_chars:z.number().int().min(500).max(50_000).optional().describe("Do not compact below this character count. Default 6000."),max_chars:z.number().int().min(1_000).max(100_000).optional().describe("Target maximum compacted character count. Default 24000.")},annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false}},async({text,threshold_chars,max_chars})=>{const result=compactText(text,{threshold_chars,max_chars});await recordApiCompaction(env,result.metrics,"chatgpt_mcp");return{content:[{type:"text",text:JSON.stringify({output:result.output,metrics:result.metrics,privacy:{content_logged_to_quillgeist_telemetry:false,aggregate_metrics_recorded:true}})}]};});
