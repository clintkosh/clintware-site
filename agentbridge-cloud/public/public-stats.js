(()=>{
  ...
  function ensureEntryActions(){
    const top=$(".top-actions");
    if(top&&!$("#installWindowsTop")){const a=document.createElement("a");a.id="installWindowsTop";a.className="btn primary";a.href=INSTALLER;a.textContent="Install for Windows";top.prepend(a)}
    if(top&&!$("#apiChatGPTTop")){const a=document.createElement("a");a.id="apiChatGPTTop";a.className="btn ghost";a.href="/developers.html";a.textContent="API + ChatGPT";top.prepend(a)}
    const home=$("#view-home .page-head .actions");
    if(home&&!$("#installWindowsHome")){const a=document.createElement("a");a.id="installWindowsHome";a.className="btn primary";a.href=INSTALLER;a.textContent="Install Windows app";home.prepend(a)}
    if(home&&!$("#apiChatGPTHome")){const a=document.createElement("a");a.id="apiChatGPTHome";a.className="btn ghost";a.href="/developers.html";a.textContent="API + ChatGPT";home.prepend(a)}
    const landing=$(".landing-actions[aria-label='Download Quillgeist']");
    if(landing&&!$("#apiChatGPTLanding")){const a=document.createElement("a");a.id="apiChatGPTLanding";a.className="btn ghost wide";a.href="/developers.html";a.textContent="API + ChatGPT";landing.append(a)}
  }
  ...
  function inject(){
    ...
    if(landingCopy&&!$("#publicImpact")){const sec=document.createElement("section");sec.id="publicImpact";sec.className="public-impact";sec.innerHTML=`<div class="kicker">Live product impact</div><h2>Measured across participating Quillgeist usage</h2>${cardMarkup("pub")}`;landingCopy.append(sec)}
    const home=$("#view-home");
    if(home&&!$("#homeGlobalImpact")){const sec=document.createElement("section");sec.id="homeGlobalImpact";sec.className="panel global-impact";sec.style.marginTop="14px";sec.innerHTML=`<div class="section-head"><div><div class="kicker">Quillgeist network</div><h2>Global impact</h2><p>Aggregate usage reported by participating installations and public API/MCP calls.</p></div></div>${cardMarkup("homeGlobal")}`;home.querySelector(".metric-grid")?.after(sec)}
  }
  ...
  function render(data){
    ...
    for(const p of ["pub","homeGlobal"]){
      ...
      setText(p+"CompactionRate",`${pct(m.compaction_rate_pct)} compacted`);setText(p+"ApiCompactions",`${fmt(m.api_compactions)} API/MCP compactions`);setText(p+"PassThrough",`${fmt(m.pass_through_runs)} pass-through`);setText(p+"Success",`${pct(success)} success`);
  ...
  inject();refresh();setInterval(()=>{if(!document.hidden)refresh()},120000);new MutationObserver(()=>ensureEntryActions()).observe(document.body,{childList:true,subtree:true});
})();
