(()=>{
  ...
  function inject(){
    ...
    if(landingCopy&&!$("#publicImpact")){const sec=document.createElement("section");sec.id="publicImpact";sec.className="public-impact";sec.innerHTML=`<div class="kicker">Live product impact</div><h2>Measured across participating Quillgeist installs</h2>${cardMarkup("pub")}`;landingCopy.append(sec)}
  ...
  inject();refresh();setInterval(()=>{if(!document.hidden)refresh()},120000);new MutationObserver(()=>ensureWindowsInstall()).observe(document.body,{childList:true,subtree:true});
})();
