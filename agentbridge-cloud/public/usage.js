(()=>{
  ...
  function derive(){
    ...
    for(const e of events){
      if(e.type!=="usage_plan_snapshot"||!e.metadata)continue;
    ...
    let external=0,local=0,saved=0,raw=0;
    const checkpointTs=new Map();
    for(const [device,e] of latestByDevice){
      const c=e.metadata?.consumption;
      if(!c)continue;
      external+=Number(c.external_tokens_used_est||0);local+=Number(c.local_tokens_used_est||0);saved+=Number(c.net_tokens_saved_est||0);raw+=Number(c.raw_tokens_est||0);checkpointTs.set(device,Number(e.ts||0));
    }
    for(const e of runs){
      const checkpoint=checkpointTs.get(e.device_id||"unknown");
      if(checkpoint&&Number(e.ts||0)<=checkpoint)continue;
      external+=Number(e.sent_tokens_est||0);local+=Number(e.local_tokens_est||0);saved+=Number(e.net_tokens_saved_est||e.tokens_avoided_est||0);raw+=Number(e.raw_tokens_est||0);
    }
  ...
  function inject(){
    ...
    if(activityBtn){const b=document.createElement("button");b.className="nav-btn";b.dataset.view="usage";b.setAttribute("aria-label","Usage");b.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M4 19h16M6 16V9m4 7V5m4 11v-4m4 4V7"/></svg><span>Usage</span>';activityBtn.before(b)}
    const home=q("#view-home"),activity=q("#view-activity");const section=document.createElement("section");section.id="view-usage";section.className="view";section.dataset.title="Usage";
    section.innerHTML=`<div class="page-head"><div><div class="kicker">Consumption + headroom</div><h1>Usage & Savings</h1><p>External consumption falls as Quillgeist compacts or handles work locally. Plan balances are shown only when their source is known.</p></div><div class="actions"><button id="usageRefresh" class="btn primary">Refresh</button></div></div><div class="usage-layout"><section class="panel usage-gauge-card glow"><div class="usage-ring" id="usageRing"><div><b id="usagePressure">0%</b><small>external pressure</small></div></div><div><div class="kicker">Lower is better</div><h2 id="usageAvoided">0% avoided</h2><p class="muted">External used ÷ (external used + net saved). The ring sinks as Quillgeist prevents external consumption.</p></div></section><section class="panel"><div class="section-head"><div><h2>Overall</h2><p>Account telemetry across paired Nodes.</p></div><span id="usageSync" class="status-chip">online</span></div><div class="usage-stat-grid"><div><span>Saved</span><b id="uSaved">0</b></div><div><span>Local</span><b id="uLocal">0</b></div><div><span>External</span><b id="uExternal">0</b></div><div><span>Local − external</span><b id="uDelta">0</b></div><div><span>Baseline external</span><b id="uBaseline">0</b></div><div><span>Local work share</span><b id="uLocalShare">0%</b></div></div></section></div><div class="usage-bars panel"><div class="section-head"><div><h2>Work distribution</h2><p>Cumulative Node checkpoints plus newer run telemetry.</p></div></div><div id="usageBars"></div></div><section class="panel" style="margin-top:14px"><div class="section-head"><div><h2>Connected plans</h2><p>Provider API when available; otherwise manual or estimated. Unknown balances stay unknown.</p></div><div><b id="uPlanCount">0</b> <span class="muted">plans</span></div></div><div id="planSummary" class="usage-plan-summary"></div><div id="usagePlans" class="usage-plans"></div></section>`;
    (activity||home).before(section);if(home){const card=document.createElement("section");card.className="panel usage-home-card";card.style.marginTop="14px";card.innerHTML='<div class="section-head"><div><div class="kicker">Usage & Savings</div><h2 id="homeUsageHeadline">0% external pressure</h2><p id="homeUsageDetail">Waiting for Quillgeist consumption telemetry.</p></div><button class="btn ghost" data-view-jump="usage">Open usage</button></div>';home.querySelector(".metric-grid")?.after(card)}
    q("#usageRefresh")?.addEventListener("click",()=>refreshUsage(true));try{commands.push(["Usage & Savings","Consumption, local work, savings, and plan headroom","usage"])}catch{}
  }
