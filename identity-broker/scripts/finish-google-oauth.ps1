...
$SetupUrl  = "https://raw.githubusercontent.com/clintkosh/clintware-site/main/newsletter/scripts/setup-gmail-oauth.ps1"
...
try {
  $Mail = Invoke-RestMethod "https://mail.clintware.com/health"
  $Mail | ConvertTo-Json -Depth 6
} catch {
  Write-Host ("Could not read mail.clintware.com health yet: " + $_.Exception.Message) -ForegroundColor Yellow
}

Write-Host ""
Write-Host "Checking ClintCal..." -ForegroundColor Cyan
try {
  $Meet = Invoke-RestMethod "https://meet.clintware.com/health"
  $Meet | ConvertTo-Json -Depth 6
} catch {
  Write-Host ("Could not read meet.clintware.com health yet: " + $_.Exception.Message) -ForegroundColor Yellow
}
