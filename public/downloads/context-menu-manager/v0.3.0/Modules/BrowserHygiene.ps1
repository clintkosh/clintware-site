...
function Get-ClintwareChromiumProfiles {
    foreach ($b in Get-ClintwareBrowserDefinitions) {
        ...
        $exe = $b.ExeCandidates | Where-Object { Test-Path -LiteralPath $_ } | Select-Object -First 1
...
function Get-ClintwareExtensionAudit {
    ...
    $highRisk = @('<all_urls>','*://*/*','debugger','management','nativeMessaging','proxy','webRequestBlocking','privacy')
    foreach ($idDir in Get-ChildItem -LiteralPath $root -Directory -ErrorAction SilentlyContinue) {
        ...
        try {
            ...
            $flags = @($perms | Where-Object { $highRisk -contains $_ -or $_ -match '^\*://|^https?://\*/|<all_urls>' })
...
function Get-ClintwareBrowserHygieneReport {
    $rows = foreach ($p in Get-ClintwareChromiumProfiles) {
        $cachePaths = @('Cache','Code Cache','GPUCache','Service Worker\CacheStorage') | ForEach-Object { Join-Path $p.Path $_ }
        $cacheMB = [math]::Round((($cachePaths | ForEach-Object { Get-ClintwareDirectorySizeMB $_ } | Measure-Object -Sum).Sum),1)
        ...
        [pscustomobject]@{
            ...
            CacheMB=$cacheMB; CookiesPresent=(Test-Path -LiteralPath (Join-Path $p.Path 'Network\Cookies'));
...
function Backup-ClintwareBrowserProfileItems {
    ...
    foreach ($rel in $RelativePaths) {
        ...
        if ((Get-Item -LiteralPath $src).PSIsContainer) { Copy-Item -LiteralPath $src -Destination $target -Recurse -Force -ErrorAction SilentlyContinue }
        else { Copy-Item -LiteralPath $src -Destination $target -Force -ErrorAction SilentlyContinue }
    }
    [pscustomobject]@{ BackupDirectory=$dest; SourceProfile=$ProfilePath; Created=(Get-Date) } | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $dest 'manifest.json') -Encoding UTF8
    $dest
}
...
function Clear-ClintwareBrowserData {
    ...
    [pscustomobject]@{ Browser=$Browser; Profile=$Profile; Backup=$backup; Removed=($targets -join '; '); Passwords='PRESERVED' }
}
...
function Open-ClintwareBrowserSearchSettings {
    ...
    [pscustomobject]@{ Browser=$Browser; RecommendedEngine=$Engine; Mode='Guided'; Note='Engine name copied to clipboard. Browser settings opened.' }
}
...
function Set-ClintwareBrowserSearchPolicy {
    ...
    if (-not $Apply) { return $preview }
    $backup = New-ClintwareRegistryBackup -RegistryPaths @($b.Policy) -Reason "Browser search policy $Browser"
    if ($Remove) {
    ...
    [pscustomobject]@{ Browser=$Browser; Engine=$Engine; Applied=$true; Backup=$backup }
}

function Get-ClintwareBrowserSecurityChecks {
    $result = @()
    foreach ($p in Get-ClintwareChromiumProfiles) {
        $extensions = @(Get-ClintwareExtensionAudit -ProfilePath $p.Path -Browser $p.Browser -Profile $p.Profile)
        foreach ($e in $extensions | Where-Object Risk -ne 'Normal') { $result += $e }
        $result += [pscustomobject]@{ Browser=$p.Browser; Profile=$p.Profile; Id='PASSWORD-STORE'; Name='Saved password store'; Version=''; Risk=$(if(Test-Path (Join-Path $p.Path 'Login Data')){'Info'}else{'Normal'}); FlaggedPermissions='Presence only; Clintware never reads saved credentials.'; PermissionCount=0 }
    }
    $result
}
