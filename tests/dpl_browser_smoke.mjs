...
const base = process.env.DPL_URL || "https://dpl.clintware.com/";
...
try {
  ...
  await page.screenshot({ path: "dpl-home-smoke.png", fullPage: true });
  if (pageErrors.length) throw new Error("Page errors:\n" + pageErrors.join("\n---\n"));
  if (consoleErrors.length) throw new Error("Console errors:\n" + consoleErrors.join("\n---\n"));
  console.log("Doppel TCE browser smoke passed.");
} catch (err) {
  try { await page.screenshot({ path: "dpl-home-smoke.png", fullPage: true }); } catch {}
  console.error("Doppel TCE browser smoke failure:", err);
  throw err;
} finally {
  await browser.close();
}
