...
import { callPerplexity, parseBrief, extractSources, DEFAULT_MODEL } from "./research.js";
import { trackAnalysis, trackConversion, trackError, trackPageView } from "./telemetry.js";
import { getProofOsSummary } from "../lib/control-plane.js";
...
function healthResponse(env) {
  return jsonResp({
    ...
    service: "proofos",
    version: VERSION,
    research: env.PERPLEXITY_API_KEY ? "configured" : "not_configured",
    telemetry: env.CLINTWARE_PRODUCT_TOKEN ? "configured" : "not_configured",
    control_plane: env.CLINTWARE_CONTROL_PLANE_URL || "https://mcp.clintware.com",
  });
}
...
async function briefResponse(request, env, ctx) {
  ...
  let research;
  try {
    research = await callPerplexity({ company, apiKey: env.PERPLEXITY_API_KEY, model });
  } catch (error) {
    // Fallback: serve a stale cached brief when the provider fails.
    if (cache) {
      try {
        ...
        if (staleHit) {
          ...
          ctx.waitUntil(
            trackAnalysis({
              ...
              fallback_used: true,
              success: true,
              error_class: error.code || "provider_error",
              input_size: company.length,
              metadata: { company: slug },
            }).catch(() => {})
          );
    ...
    const detail = status === 503
      ? "Live research is not configured on this deployment yet."
      : "Live research failed. Try again shortly.";
    return withSession(jsonResp({ error: "research_failed", error_class: error.code || "provider_error", detail, request_id }, status, { "x-proofos-request-id": request_id }), setCookie);
  }
...
async function summaryResponse(env) {
  if (!env.CLINTWARE_PRODUCT_TOKEN) {
    return jsonResp({ ok: false, detail: "telemetry not configured" }, 200);
  }
  try {
