...
Repository CI runs those checks plus the browser BuyerOrigin tests, Worker configuration dry-runs, and `git diff --check` before the branch can be treated as ready for Shopify registration work.  Final validation is performed after public BuyerOrigin positioning is reconciled with current `main`.
