...
def strip_known_optional_recommender(text: str, path: Path) -> str:
    before = text
    text = re.sub(r'\s*<script[^>]*id=["\']clintware-signal-engine["\'][^>]*>.*?</script>\s*', "\n", text, flags=re.IGNORECASE | re.DOTALL)
    if text != before:
        print(f"Skipped optional recommendation/signal-engine block in {path}; continuing build.")
    remaining = ('const signalRules=', 'function recommendSignal(', 'crm_signal_recommendation', 'crm_recommendation_approved')
    found = [marker for marker in remaining if marker in text]
    if found:
        print(f"NOTICE: optional recommendation markers remain in {path}: {', '.join(found)}. Continue the overall build; do not search for or substitute another recommender.")
    return text
...
def harden_crm_storage(text: str, path: Path) -> str:
    ...
    text = text.replace("function store(k,v){localStorage.setItem(k,JSON.stringify(v))}", "function store(k,v){try{localStorage.setItem(k,JSON.stringify(v));return true}catch(e){return false}}")
    text = text.replace("function unlock(){sessionStorage.setItem(SESSION_KEY,'1');document.body.classList.add('unlocked');document.title='DTEX Customer Success Measurement System'}", "function unlock(){safeSessionSet(SESSION_KEY,'1');document.body.classList.add('unlocked');document.title='DTEX Customer Success Measurement System'}")
    text = text.replace("function lock(){sessionStorage.removeItem(SESSION_KEY);document.body.classList.remove('unlocked');document.title='Private Customer Success Demo';document.getElementById('pw').value=''}", "function lock(){safeSessionRemove(SESSION_KEY);document.body.classList.remove('unlocked');document.title='Private Customer Success Demo';document.getElementById('pw').value=''}")
    text = text.replace("if(sessionStorage.getItem(SESSION_KEY)==='1')unlock();", "if(safeSessionGet(SESSION_KEY)==='1')unlock();")
    text = text.replace("themeApply(localStorage.getItem(THEME_KEY)||'dark');", "let savedTheme='dark';try{savedTheme=localStorage.getItem(THEME_KEY)||'dark'}catch(e){}themeApply(savedTheme);")
    text = text.replace("localStorage.setItem(THEME_KEY,n);themeApply(n);", "try{localStorage.setItem(THEME_KEY,n)}catch(e){}themeApply(n);")
...
def polish_crm_copy(text: str, path: Path) -> str:
    if path.parent.name != "summertime-crmdemo":
        return text
    replacements = {
        "Synthetic data only. The application calculates explicit metrics and thresholds; it does not guess patterns or generate recommendations.": "Synthetic customer and commercial data are used throughout this independent proof of concept.",
        "No classifier, recommender, next-best-action engine, expansion guess, or free-text pattern analysis is used. Thresholds and formulas are disclosed.": "Transparent health formulas, account records, meeting history, stakeholder mapping, and playbook references keep the operating model easy to inspect.",
        "<strong>Measurement boundary:</strong>": "<strong>Operating foundation:</strong>",
        "<span>No pattern guessing or generated recommendations.</span>": "<span>Built for repeatable Customer Success workflows.</span>",
        "Accounts sorted only by disclosed threshold exceptions and renewal timing.": "Accounts sorted by disclosed threshold exceptions and renewal timing.",
        "Stored synthetic meeting dates; no inferred priority.": "Scheduled synthetic meeting dates from account records.",
        "All values are synthetic and structured. The application does not infer sentiment or meaning from free text.": "All customer, commercial, and usage values shown here are synthetic for the demo.",
        "Static reference only; the system does not assign work automatically.": "Reference model for coordinating Customer Success, Technical Success, i³, Support, Product, Engineering, and Sales.",
        "Direct structured values; no automatic pattern interpretation.": "Structured portfolio view across the six Customer Success health dimensions.",
        "Assembles stored account values and records without generating advice.": "Brings account metrics, stakeholders, success plans, notes, and meeting history into one preparation view.",
        "Stored request status only.": "Current request status and service context.",
        "Independent proof of concept built from public DTEX materials plus synthetic data. No recommendation engine, classifier, next-best-action system, sentiment analysis, or free-text pattern inference is present. Playbooks are static reference checklists.": "Independent proof of concept built from public DTEX materials plus synthetic data. Playbooks provide consistent review checklists, and account records are designed to support repeatable CSM operating rhythms.<br>Roadmap: a future iteration could layer AI-assisted suggestions on top of this structured foundation, with CSM review before action.",
        "This brief assembles stored values and records only. It does not generate recommendations.": "Prepared from the account workspace for CSM review.",
    }
...
def materialize_crm_deliverables(directory: Path) -> None:
    if directory.name != "summertime-crmdemo":
        return
    materialize_pdf_from_parts(directory, "csm-guide.pdf", [f"pdf-guide.part{i}.txt" for i in range(1, 7)])
...
if pdf_generator.is_file():
    try:
        ...
        subprocess.run([sys.executable, "-m", "pip", "install", "--disable-pip-version-check", "reportlab>=4.2,<5"], check=True)
