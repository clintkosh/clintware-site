...
import { enhanceFunctionalCards } from "./functional-cards.js";
...
const GA_HEAD = `<script async src="https://www.googletagmanager.com/gtag/js?id=G-DCY144YM9P"></script><script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments)};(function(){var key='an_demo_attribution_v1',saved={};try{saved=JSON.parse(localStorage.getItem(key)||'{}')}catch(e){}var p=new URLSearchParams(location.search);['utm_source','utm_medium','utm_campaign','utm_term','utm_content'].forEach(function(k){if(p.get(k))saved[k]=p.get(k)});try{localStorage.setItem(key,JSON.stringify(saved))}catch(e){}gtag('js',new Date());gtag('config','G-DCY144YM9P',{anonymize_ip:true,demo_name:'abnormal_cs_command_center',hostname:location.hostname,page_path:location.pathname,campaign_source:saved.utm_source||undefined,campaign_medium:saved.utm_medium||undefined,campaign_name:saved.utm_campaign||undefined});})();</script>`;
const DEMO_VISIT_SCRIPT = `<script id="cw-demo-visit-signal">(function(){window.dataLayer=window.dataLayer||[];window.gtag=window.gtag||function(){window.dataLayer.push(arguments)};var a={};try{a=JSON.parse(localStorage.getItem('an_demo_attribution_v1')||'{}')}catch(e){}window.gtag('event','demo_visit',{demo_name:'abnormal_cs_command_center',hostname:location.hostname,page_path:location.pathname,authenticated:true,demo_gate_success:true,campaign_source:a.utm_source,campaign_medium:a.utm_medium,campaign_name:a.utm_campaign});})();</script>`;
let appCache;
...
function loginPage(error = "") {
  ...
  return new Response(html.replace("</title>", `</title>${GA_HEAD}`), { status: error ? 401 : 200, headers: responseHeaders() });
}
...
async function loadApp() {
  ...
  appCache = addVisitSignal(enhanceFunctionalCards(enhanceAccountUsers(enhanceApp(baseApp))));
  return appCache;
}
