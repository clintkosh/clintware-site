$ErrorActionPreference = "Stop"
$ProgressPreference = "SilentlyContinue"

function Log([string]$Message) { Write-Host ("BITNET // " + $Message) }
function FreeGB([string]$Drive) {
  $p = Get-PSDrive $Drive -ErrorAction SilentlyContinue
  if ($p) { return [math]::Round($p.Free / 1GB,2) }
  return 0
}
function Find-VsDevCmd {
  @(
    "C:\Program Files\Microsoft Visual Studio\2022\BuildTools\Common7\Tools\VsDevCmd.bat",
    "C:\Program Files\Microsoft Visual Studio\2022\Community\Common7\Tools\VsDevCmd.bat",
    "C:\Program Files\Microsoft Visual Studio\2022\Professional\Common7\Tools\VsDevCmd.bat",
    "C:\Program Files\Microsoft Visual Studio\2022\Enterprise\Common7\Tools\VsDevCmd.bat"
  ) | Where-Object { Test-Path $_ } | Select-Object -First 1
}
function Resolve-RealPython {
  $rows = New-Object System.Collections.Generic.List[object]
  $miniconda = Join-Path $env:USERPROFILE "Miniconda3\python.exe"
  if (Test-Path $miniconda) {
    $rows.Add([pscustomobject]@{ Path=$miniconda; Prefix=@() })
  }

  foreach ($name in @("python.exe","py.exe","python3.exe")) {
    $cmd = Get-Command $name -ErrorAction SilentlyContinue
    if (-not $cmd -or -not $cmd.Source) { continue }
    if ([string]$cmd.Source -match '(?i)\\WindowsApps\\') { continue }
    $prefix = if ((Split-Path $cmd.Source -Leaf).ToLowerInvariant() -eq "py.exe") { @("-3.10") } else { @() }
    $rows.Add([pscustomobject]@{ Path=[string]$cmd.Source; Prefix=$prefix })
  }

  $seen = @{}
  foreach ($row in $rows) {
    $key = ([string]$row.Path).ToLowerInvariant()
    if ($seen.ContainsKey($key)) { continue }
    $seen[$key] = $true

    try {
      $args = @($row.Prefix) + @("--version")
      $version = (& $row.Path @args 2>&1 | Out-String).Trim()
      if ($LASTEXITCODE -ne 0) { continue }
      if ($version -notmatch '^Python\s+(\d+)\.(\d+)') { continue }
      $major = [int]$matches[1]
      $minor = [int]$matches[2]
      if ($major -gt 3 -or ($major -eq 3 -and $minor -ge 10)) {
        return [pscustomobject]@{ Path=[string]$row.Path; Prefix=@($row.Prefix); Version=$version }
      }
    } catch {}
  }

  throw "A working Python 3.10+ runtime was not found; Windows Store aliases are ignored."
}
function Get-FreeTcpPort {
  $listener = [Net.Sockets.TcpListener]::new([Net.IPAddress]::Loopback,0)
  $listener.Start()
  try { return ([Net.IPEndPoint]$listener.LocalEndpoint).Port }
  finally { $listener.Stop() }
}

Log "Checking storage and prerequisites without moving user files"
$cFree = FreeGB "C"
$fFree = FreeGB "F"
if ($cFree -lt 8) {
  throw "C: has only $cFree GB free. BitNet setup will not move or delete files automatically."
}

$f = Get-PSDrive F -ErrorAction SilentlyContinue
if ($f -and $f.Free -gt 12GB) {
  $Root = "F:\AI-Data\BitNet"
} elseif ($cFree -ge 20) {
  $Root = "C:\AI\BitNet"
} else {
  throw "F: does not have 12 GB free and C: does not have 20 GB free. No files were moved."
}

$gitCmd = Get-Command git.exe -ErrorAction SilentlyContinue
if (-not $gitCmd) { $gitCmd = Get-Command git -ErrorAction SilentlyContinue }
if (-not $gitCmd -or -not $gitCmd.Source) { throw "Git is required." }
$git = [string]$gitCmd.Source

$python = Resolve-RealPython
Log ("Python " + $python.Version + " @ " + $python.Path)
Log ("Storage C=" + $cFree + " GB F=" + $fFree + " GB Root=" + $Root)

$vs = Find-VsDevCmd
if (-not $vs) {
  $wingetCmd = Get-Command winget.exe -ErrorAction SilentlyContinue
  if (-not $wingetCmd) { $wingetCmd = Get-Command winget -ErrorAction SilentlyContinue }
  if (-not $wingetCmd -or -not $wingetCmd.Source) {
    throw "VS2022 C++/Clang prerequisites are missing and winget is unavailable."
  }
  Log "Installing VS2022 Build Tools C++/Clang prerequisites"
  & $wingetCmd.Source install --id Microsoft.VisualStudio.2022.BuildTools --exact --source winget --silent --accept-package-agreements --accept-source-agreements --override "--wait --passive --norestart --add Microsoft.VisualStudio.Workload.VCTools --includeRecommended --add Microsoft.VisualStudio.Component.VC.Llvm.Clang"
  if ($LASTEXITCODE -ne 0 -and $LASTEXITCODE -ne -1978335189) {
    throw "VS2022 Build Tools install failed: $LASTEXITCODE"
  }
  $vs = Find-VsDevCmd
}
if (-not $vs) { throw "VS2022 Developer environment is unavailable." }

New-Item -ItemType Directory -Force -Path (Split-Path $Root -Parent) | Out-Null

if (-not (Test-Path (Join-Path $Root ".git"))) {
  Log "Cloning official microsoft/BitNet"
  & $git clone --recursive https://github.com/microsoft/BitNet.git $Root
  if ($LASTEXITCODE -ne 0) { throw "BitNet clone failed." }
} else {
  Log "Refreshing existing official BitNet checkout"
  & $git -C $Root pull --ff-only
  if ($LASTEXITCODE -ne 0) { throw "BitNet pull failed." }
  & $git -C $Root submodule update --init --recursive
  if ($LASTEXITCODE -ne 0) { throw "BitNet submodule update failed." }
}

$Venv = Join-Path $Root ".venv"
$VenvPy = Join-Path $Venv "Scripts\python.exe"
if (-not (Test-Path $VenvPy)) {
  $venvArgs = @($python.Prefix) + @("-m","venv",$Venv)
  & $python.Path @venvArgs
  if ($LASTEXITCODE -ne 0) { throw "BitNet Python environment creation failed." }
}
if (-not (Test-Path $VenvPy)) { throw "BitNet virtual environment Python is missing." }

& $VenvPy -m pip install --disable-pip-version-check -r (Join-Path $Root "requirements.txt")
if ($LASTEXITCODE -ne 0) { throw "BitNet requirements install failed." }
& $VenvPy -m pip install --disable-pip-version-check huggingface_hub
if ($LASTEXITCODE -ne 0) { throw "Hugging Face client install failed." }

$ModelDir = Join-Path $Root "models\BitNet-b1.58-2B-4T"
$Model = Join-Path $ModelDir "ggml-model-i2_s.gguf"
if (-not (Test-Path $Model)) {
  New-Item -ItemType Directory -Force -Path $ModelDir | Out-Null
  $hf = @(
    (Join-Path $Venv "Scripts\hf.exe"),
    (Join-Path $Venv "Scripts\huggingface-cli.exe")
  ) | Where-Object { Test-Path $_ } | Select-Object -First 1
  if (-not $hf) { throw "Hugging Face CLI is unavailable." }

  Log "Downloading official BitNet model"
  & $hf download microsoft/BitNet-b1.58-2B-4T-gguf --local-dir $ModelDir
  if ($LASTEXITCODE -ne 0) { throw "BitNet model download failed." }
}
if (-not (Test-Path $Model)) { throw "Expected BitNet GGUF was not found after model setup." }

Log "Building official BitNet Windows runtime"
$cmd = 'call "' + $vs + '" -startdir=none -arch=x64 -host_arch=x64 && cd /d "' + $Root + '" && "' + $VenvPy + '" setup_env.py -md "' + $ModelDir + '" -q i2_s'
& cmd.exe /d /s /c $cmd
if ($LASTEXITCODE -ne 0) { throw "BitNet build failed: $LASTEXITCODE" }

[Environment]::SetEnvironmentVariable("QUILLGEIST_BITNET_HOME",$Root,"User")
$env:QUILLGEIST_BITNET_HOME = $Root

$cli = @(
  (Join-Path $Root "build\bin\Release\llama-cli.exe"),
  (Join-Path $Root "build\bin\llama-cli.exe")
) | Where-Object { Test-Path $_ } | Select-Object -First 1
$server = @(
  (Join-Path $Root "build\bin\Release\llama-server.exe"),
  (Join-Path $Root "build\bin\llama-server.exe")
) | Where-Object { Test-Path $_ } | Select-Object -First 1

if (-not $cli) { throw "BitNet build completed without llama-cli." }
if (-not $server) { throw "BitNet build completed without llama-server." }

Log "Running BitNet CLI validation generation"
$out = & $cli -m $Model -p "Reply with exactly BITNET_READY." -n 32 -c 2048 -t ([math]::Max(2,[math]::Min(8,[Environment]::ProcessorCount))) --no-display-prompt 2>&1 | Out-String
if ($LASTEXITCODE -ne 0 -or -not $out.Trim()) { throw "BitNet CLI validation failed." }

Log "Running temporary BitNet HTTP server round trip"
$port = Get-FreeTcpPort
$serverArgs = @(
  "-m",$Model,
  "-c","2048",
  "-t",[string]([math]::Max(2,[math]::Min(8,[Environment]::ProcessorCount))),
  "-ngl","0",
  "--host","127.0.0.1",
  "--port",[string]$port
)
$serverProc = Start-Process -FilePath $server -ArgumentList $serverArgs -WindowStyle Hidden -PassThru
$roundTrip = $null
try {
  $ready = $false
  $deadline = (Get-Date).AddSeconds(75)
  do {
    Start-Sleep -Milliseconds 750
    try {
      $health = Invoke-RestMethod -Uri ("http://127.0.0.1:" + $port + "/health") -TimeoutSec 3
      if ($health) { $ready = $true; break }
    } catch {}
    try { if ($serverProc.HasExited) { break } } catch {}
  } while ((Get-Date) -lt $deadline)

  if (-not $ready) { throw "BitNet llama-server did not become healthy." }

  $payload = @{
    prompt = "Reply with exactly BITNET_SERVER_READY."
    n_predict = 32
    temperature = 0
  } | ConvertTo-Json -Depth 4

  $response = Invoke-RestMethod -Method Post -Uri ("http://127.0.0.1:" + $port + "/completion") -ContentType "application/json" -Body $payload -TimeoutSec 90
  $content = [string]$response.content
  if (-not $content.Trim()) { throw "BitNet llama-server returned an empty completion." }

  $roundTrip = [ordered]@{
    ok = $true
    port = $port
    output_tail = $content.Substring([math]::Max(0,$content.Length-1000))
  }
} finally {
  try {
    if ($serverProc -and -not $serverProc.HasExited) {
      Stop-Process -Id $serverProc.Id -Force -ErrorAction SilentlyContinue
    }
  } catch {}
}

[ordered]@{
  ok = $true
  root = $Root
  model = $Model
  cli = $cli
  server = $server
  python = $python.Path
  validation_tail = $out.Substring([math]::Max(0,$out.Length-2000))
  server_round_trip = $roundTrip
  moved_files = 0
  d_drive_touched = $false
} | ConvertTo-Json -Depth 8
