...
function Start-ControlPlaneDeploy {
  param([object]$Gh)
  Write-Host ""
  Write-Host "Deploying Jira credentials into the Clintware Control Plane..." -ForegroundColor Cyan
  & $Gh.Source workflow run $DeployWorkflow --repo $Repo
  if ($LASTEXITCODE -ne 0) {
    throw "Could not start the Clintware Control Plane deployment workflow."
  }
}
...
function Wait-JiraConfigured {
  ...
  throw "Jira credentials did not become active before timeout. Check the Deploy Clintware Control Plane workflow."
}
